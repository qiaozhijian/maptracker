...
def parse_args():
    ...
    parser.add_argument(
        "--data-root",
        type=str,
        default="/home/qzj/datasets/argoverse2/sensor",
...
def create_av2_infos_mp(
    ...
    train_loader = AV2SensorDataLoader(
        Path(osp.join(root_path, "train")), Path(osp.join(root_path, "train"))
    )
    val_loader = AV2SensorDataLoader(
        Path(osp.join(root_path, "val")), Path(osp.join(root_path, "val"))
    )
    test_loader = AV2SensorDataLoader(
        Path(osp.join(root_path, "test")), Path(osp.join(root_path, "test"))
    )
    ...
    for log_id in log_ids:
        ...
        map_fname = os.path.basename(
            str(list(Path(map_path_dir).glob("log_map_archive_*.json"))[0])
        )
    ...
    if newsplit:
        info_path = osp.join(
            dest_path, "{}_map_infos_{}_newsplit.pkl".format(info_prefix, split)
        )
    else:
        info_path = osp.join(
            dest_path, "{}_map_infos_{}.pkl".format(info_prefix, split)
        )
...
if __name__ == "__main__":
    ...
    dest_path = os.path.join(args.data_root, "custom", "maptracker")
