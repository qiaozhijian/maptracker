    ...
    head_cfg=dict(
        ...
        assigner=dict(
            ...
            cost=dict(
                ...
                reg_cost=dict(
                    type="LinesL1Cost", weight=50.0, beta=0.01, permute=permute
                ),
            ),
        ),
    ),
...
eval_config = dict(
    ...
    ann_file="/home/qzj/datasets/nuscenes/custom/maptracker/nuscenes_map_infos_val_newsplit.pkl",
    meta=meta,
    roi_size=roi_size,
    cat2id=cat2id,
    pipeline=[
...
data = dict(
    ...
    train=dict(
        type="NuscDataset",
        data_root="./datasets/nuscenes",
        ann_file="/home/qzj/datasets/nuscenes/custom/maptracker/nuscenes_map_infos_train_newsplit.pkl",
        meta=meta,
        roi_size=roi_size,
        cat2id=cat2id,
...
find_unused_parameters = (
    True  #### when use checkpoint, find_unused_parameters must be False
)
checkpoint_config = dict(interval=num_epochs_interval * num_iters_per_epoch)

runner = dict(type="MyRunnerWrapper", max_iters=num_epochs * num_iters_per_epoch)
...
load_from = (
    "work_dirs/maptracker_nusc_newsplit_5frame_span10_stage1_bev_pretrain/latest.pth"
)
